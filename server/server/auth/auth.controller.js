...
function invalidRegisterRequest(res) {
  //const message = `Registro invalido. . ${httpStatus["400_MESSAGE"]}`;
  const message = `Registro invalido. ` 
...
function incorrectLoginCredentials(res) {
  //const message = `Nombre de usuario o contraseña incorrectos. ${httpStatus["401_MESSAGE"]}`
  const message = `Nombre de usuario o contraseña incorrectos`
...
function failedToRegister(res) {
  ...
  console.log(`fallaregistro( ${message} )`);
  return util.createErrorResponse(res, httpStatus.BAD_REQUEST, message);
}
